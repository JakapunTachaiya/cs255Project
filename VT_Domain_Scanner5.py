...
_api1 = '7f908c8969921575152d4826e2e430db78fce659ced443322eeb6e0f3c5329de'
_api2 = '2f0caa4ad1380f9261b7d3ae37e7af8b26230155eb87a1675d3bc65e9b7c1675'
_api3 = '0bb156544c7170871cdf5377853683489ee70873a3d005d3d67a771c5a9fa56c'
_api4 = 'bf97692affc8865caa7d2bedca0b4049a416f22a9701ff0b7ef89d642f24b1eb'
_api5 = '7d31cc35dd56aab3be7ed27a6dd2e02037208d3194495fb390ab83cb8b14e56a'
_api6 = '1e353347d42cc7bb67d707883c04c270e8d91811e42f29b6b2515aa36ae0b704'
_api7 = '33985fdbc4959737062e2497c6903ed62e11fcfbb84b436df754ad9f4a871e0e'
_api8 = 'f92a41515c6bfcb86bfdcf8760169b25b6f971377e2e850d7dd3c13928990256'
_api9 = 'c34870696cd4edb4277802dd24a4688bc0239e136d5944e655ba760e4500834b'
_api10 = '08ac28d97d6c5fe22660e92abef363fe9d40bb7c687445f106f7366b776c87a5'
_inputPath = 'offCommURL.txt'
_outputPath = 'offCommURL_label.txt'


# submits domain to VT to generate a fresh report for DomainReportReader()
def DomainScanner(domain, api):
...
def DomainReportReader(domain, api):
    ...
    # resultsString = (
    #             domain + ' was scanned on ' + scanDate + ' and contained a ' + ratio + ' AV detection ratio. See full report in results file for further information')
    # print(resultsString)
    resultsOutput = str(domain) + ',virusratio[' + ratio + '],virusLink[' + str(permalink) + ']' + '\n'
...
badDomains = open(_inputPath, 'r')  ###### change path for input file
...
while True:
    ...
    results = open(_outputPath, 'a')  ###### change path for input file
    domain1 = badDomains.readline().rstrip('\n')
    domain2 = badDomains.readline().rstrip('\n')

    if not domain2:
        break

    print('count: ', count, 'Passing ', domain1)
    DomainScanner(domain1, _api9)
    count = count + 1
    print('count: ', count, 'Passing ', domain2)
    count = count + 1
    DomainScanner(domain2, _api10)

    time.sleep(15)
    DomainReportReader(domain1, _api9)
    time.sleep(15)
    DomainReportReader(domain2, _api10)
    results.close()
